                ...
                success: function(data){
                    ...
                    if (data_main.login_success && data_main.redirect) {
                        if(data_main.redirect != '1') {
                            $("#loginRegisterPopup .oe_login_form .alert-success-error").html('<p class="alert alert-success">You are successfully logged in</p>')
                        ...
                        $("#loginRegisterPopup .oe_login_form .alert-success-error").html('<p class="alert alert-danger">'+data_main.error+'</p>')
                    }
                },
                error: function(data) {
                    console.log(data)
        ...
        _customerRegistration: function(e){
            ...
            $.ajax({
                ...
                success: function(data){
                    var data_main = JSON.parse(data)
                    if (data_main.login_success && data_main.redirect) {
                        $("#loginRegisterPopup .oe_signup_form .alert-success-error").html('<p class="alert alert-success">You are successfully register your account</p>')
                        window.location.reload()
                    } else if (!data_main.login_success && data_main.error) {
                        $("#loginRegisterPopup .oe_signup_form .alert-success-error").html('<p class="alert alert-danger">'+data_main.error+'</p>')
                    }
                },
                error: function(data) {
                    console.log('An error occurred.')
